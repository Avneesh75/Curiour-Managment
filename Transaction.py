...
def main_dashboard(username, branch_code):
    root = tk.Tk()
    ...
    # Logout button on the left (with softer red)
    user_logout = tk.Button(top_frame, text="Logout", font=("Arial", 10, 'bold'),bg="#f05b5b", fg="white", activebackground="#d43f3f", activeforeground="white",relief="flat", padx=10, command=root.destroy)
    user_logout.pack(side=tk.RIGHT, padx=10, pady=2)

    # Welcome message aligned to the right
    user_label = tk.Label(top_frame,text=f"Welcome {username} | Branch: {branch_code}",font=('Arial', 10, 'bold'), fg='purple', bg="aliceblue")
...
def build_booking_gui():
    ...
    consignee_address_entry = tk.Entry(right_frame, width=65)
    consignee_address_entry.place(x=130, y=170)

    tk.Label(right_frame, text="Address 2", bg="lightgrey").place(x=10, y=200)
    consignee_address2_entry = tk.Entry(right_frame, width=65)
